...
import {
    ...
    Calendar, Award, GraduationCap, Building2,
    Globe, Briefcase, CheckCircle2, Download,
} from "lucide-react";
import { useAuth } from "@/contexts/AuthContext";
import { resumeService, educationService, skillService, workExperienceService } from "@/services/api";
...
function calcDurationLabel(startYear?: number, startMonth?: number, endYear?: number, endMonth?: number, isCurrent?: boolean): string | null {
    ...
    let months = (endY - startYear) * 12 + (endM - startM);
...
export default function ViewResumePage() {
    ...
    const handleDownloadPdf = async () => {
        try {
            await resumeService.exportPdf(Number(params.id));
        } catch {
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <Link href="/jobseeker/resumes">
                        <button className="btn-ghost flex items-center gap-2 text-sm">
                            <ArrowLeft className="h-4 w-4" /> Мои резюме
                        </button>
                    </Link>
                    {userRole === "JOB_SEEKER" && (
                        <div className="flex items-center gap-2.5">
                            <span className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full ${
                                ...
                                {resume.isPublished
                                    ...
                                    : <><Globe className="w-3 h-3" /> Черновик</>
                                }
                            </span>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-4 items-start">
                                ...
                                <div className="space-y-3">
                                    {resume.contactEmail && (
                                        <a href={`mailto:${resume.contactEmail}`}
                                            className="flex items-center gap-3 group text-sm hover:underline"
                    ...
                    <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}
                        ...
                        {resume.summary && (
                            <div className="card p-6">
                                <SectionTitle icon={Globe} label="О себе" />
